...
function PanelPagos({ estudioId, onBack }: any) { const [actuaciones, setActuaciones] = useState<any[]>([]); const [casos, setCasos] = useState<any[]>([]); const [loading, setLoading] = useState(true); const [filtro, setFiltro] = useState<'recibidos' | 'coordinar' | 'arquitecto'>('recibidos'); useEffect(() => { fetchData(); }, []); const fetchData = async () => { try { const casosRef = collection(db, 'Estudios', estudioId, 'Casos'); const casosSnap = await getDocs(casosRef); const casosData = casosSnap.docs.map(d => ({ id: d.id, ...d.data() })); setCasos(casosData); const todasActuaciones: any[] = []; for (const caso of casosData) { const actRef = collection(db, 'Estudios', estudioId, 'Casos', caso.id, 'Actuaciones'); const actSnap = await getDocs(actRef); actSnap.docs.forEach(d => { todasActuaciones.push({ id: d.id, caseId: caso.id, casoData: caso, ...d.data() }); }); } setActuaciones(todasActuaciones); } catch (e) { console.error('Error:', e); } setLoading(false); }; const marcarPagadoUsuario = async (caseId: string, actId: string) => { await updateDoc(doc(db, 'Estudios', estudioId, 'Casos', caseId, 'Actuaciones', actId), { pago_usuario: 'pagado' }); setActuaciones(prev => prev.map(a => a.id === actId ? { ...a, pago_usuario: 'pagado' } : a)); }; const marcarPagadoArquitecto = async (caseId: string, actId: string) => { await updateDoc(doc(db, 'Estudios', estudioId, 'Casos', caseId, 'Actuaciones', actId), { pago_arquitecto: 'pagado' }); setActuaciones(prev => prev.map(a => a.id === actId ? { ...a, pago_arquitecto: 'pagado' } : a)); }; const recibidos = actuaciones.filter(a => a.metodo_pago && a.metodo_pago !== 'Internacional' && a.estado !== 'coordinando_pago'); const coordinar = actuaciones.filter(a => a.metodo_pago === 'Internacional' || a.estado === 'coordinando_pago'); const conHonorario = actuaciones.filter(a => a.honorario_arquitecto && a.honorario_arquitecto > 0); const totalIngresos = recibidos.filter(a => a.pago_usuario === 'pagado').reduce((sum, a) => sum + (parseInt((a.precio||'').replace(/[^0-9]/g,'')) || 0), 0); const totalPendienteArq = conHonorario.filter(a => a.pago_arquitecto !== 'pagado').reduce((sum, a) => sum + (a.honorario_arquitecto || 0), 0); const lista = filtro === 'recibidos' ? recibidos : filtro === 'coordinar' ? coordinar : conHonorario; return (<div style={styles.container}><div style={styles.engineeringHeader}><button onClick={onBack} style={styles.btnBack}>← Volver</button><span>Tesorería</span></div><h2 style={styles.h2}>PANEL DE PAGOS</h2><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '20px' }}><div style={{ padding: '12px', backgroundColor: '#FFFFFF', borderRadius: '8px', border: THEME.border }}><p style={{ fontSize: '10px', fontWeight: 700, color: THEME.gray, marginBottom: '4px' }}>INGRESOS COBRADOS</p><p style={{ fontSize: '18px', fontWeight: 900, color: '#2E7D32' }}>${totalIngresos.toLocaleString('es-UY')}</p></div><div style={{ padding: '12px', backgroundColor: '#FFFFFF', borderRadius: '8px', border: THEME.border }}><p style={{ fontSize: '10px', fontWeight: 700, color: THEME.gray, marginBottom: '4px' }}>HONORARIOS PENDIENTES</p><p style={{ fontSize: '18px', fontWeight: 900, color: '#B21F24' }}>${totalPendienteArq.toLocaleString('es-UY')}</p></div></div><div style={{ display: 'flex', gap: '6px', marginBottom: '20px' }}><button onClick={() => setFiltro('recibidos')} style={{ flex: 1, padding: '10px', fontSize: '10px', fontWeight: 900, border: filtro === 'recibidos' ? '2px solid #1D1D1F' : '1px solid #E5E5E7', backgroundColor: filtro === 'recibidos' ? '#1D1D1F' : '#FFFFFF', color: filtro === 'recibidos' ? '#FFFFFF' : '#1D1D1F', borderRadius: '6px', cursor: 'pointer' }}>Recibidos ({recibidos.length})</button><button onClick={() => setFiltro('coordinar')} style={{ flex: 1, padding: '10px', fontSize: '10px', fontWeight: 900, border: filtro === 'coordinar' ? '2px solid #1D1D1F' : '1px solid #E5E5E7', backgroundColor: filtro === 'coordinar' ? '#1D1D1F' : '#FFFFFF', color: filtro === 'coordinar' ? '#FFFFFF' : '#1D1D1F', borderRadius: '6px', cursor: 'pointer' }}>Coordinar ({coordinar.length})</button><button onClick={() => setFiltro('arquitecto')} style={{ flex: 1, padding: '10px', fontSize: '10px', fontWeight: 900, border: filtro === 'arquitecto' ? '2px solid #1D1D1F' : '1px solid #E5E5E7', backgroundColor: filtro === 'arquitecto' ? '#1D1D1F' : '#FFFFFF', color: filtro === 'arquitecto' ? '#FFFFFF' : '#1D1D1F', borderRadius: '6px', cursor: 'pointer' }}>Al arquitecto ({conHonorario.length})</button></div>{loading ? <p style={{ color: THEME.gray }}>Cargando...</p> : lista.length === 0 ? (<div style={{ ...styles.cardInfo, border: THEME.border, textAlign: 'center' }}><p style={{ color: THEME.gray }}>No hay registros en esta categoría.</p></div>) : (<div style={{ display: 'flex', flexDirect
