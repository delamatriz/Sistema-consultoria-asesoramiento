...
function ScreenDetalle({ caseId, onBack, onEscalate }: any) {
  ...
  return (
    <div style={styles.container}>
      ...
      <button onClick={onEscalate} style={{ ...styles.btnPrimary }}>Ver opciones de asesoramiento</button> {actuaciones.length > 0 && <div style={{ marginTop: '20px' }}><label style={styles.label}>HISTORIAL DE ACTUACIONES</label>{actuaciones.map(a => <div key={a.id} style={{ ...styles.cardInfo, border: '2px solid #E5E5E7', marginBottom: '12px' }}><p style={{ fontSize: '11px', fontWeight: 900, color: '#6E6E73', margin: '0 0 4px 0' }}>{a.nivel_servicio}</p><p style={{ fontSize: '14px', lineHeight: '1.6', margin: '0 0 10px 0' }}>{a.respuesta_texto}</p>{a.recomendacion && <p style={{ fontSize: '12px', color: '#B21F24', fontWeight: 700, margin: 0 }}>Recomendacion: {a.recomendacion}</p>}</div>)}</div>}
    </div>
  );
}
