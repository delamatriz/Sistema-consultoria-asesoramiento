...
function PanelADashboard({ currentUser, userProfile, onCase, onLogout, onBiblioteca, onMisCasos, onMiCuenta, onMiPerfil }: any) {
  ...
  return (
    <div style={styles.container}>
      ...
      {/* Casos recientes */}
      <label style={styles.label}>CASOS RECIENTES</label><div style={{ display: 'flex', gap: '10px', margin: '10px 0' }}><button onClick={() => { setVerArchivados(false); fetchData(); }} style={{ padding: '6px 14px', fontSize: '11px', fontWeight: 900, borderRadius: '4px', border: '1px solid #B21F24', backgroundColor: verArchivados ? 'transparent' : '#B21F24', color: verArchivados ? '#B21F24' : '#FFFFFF', cursor: 'pointer', flex: 1 }}>ACTIVOS</button><button onClick={() => { setVerArchivados(true); fetchData(); }} style={{ padding: '6px 14px', fontSize: '11px', fontWeight: 900, borderRadius: '4px', border: '1px solid #B21F24', backgroundColor: verArchivados ? '#B21F24' : 'transparent', color: verArchivados ? '#FFFFFF' : '#B21F24', cursor: 'pointer', flex: 1 }}>ARCHIVADOS</button></div>
