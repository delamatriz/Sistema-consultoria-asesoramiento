...
function ScreenDetalle({ caseId, onBack, onEscalate, onAsignar }: any) {
  ...
  return (
    <div style={styles.container}>
      ...
      <button onClick={onEscalate} style={{ ...styles.btnPrimary }}>Ver opciones de asesoramiento</button> {actuaciones.filter(a => a.estado === 'entregada' || a.estado === 'completada').length > 0 && <div style={{ marginTop: '20px' }}><label style={{ fontSize: '11px', fontWeight: 900, letterSpacing: '0.1em', color: '#B21F24', display: 'block', marginBottom: '12px' }}>ACTUACIONES RECIBIDAS</label>{actuaciones.filter(a => a.estado === 'entregada' || a.estado === 'completada').map(a => <div key={a.id} style={{ ...styles.cardInfo, border: '2px solid #E5E5E7', marginBottom: '12px' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}><p style={{ fontSize: '11px', fontWeight: 900, color: '#6E6E73', margin: 0 }}>{a.nivel_servicio}{' — '}{a.nombre_servicio||''}</p><span style={{ fontSize: '11px', fontWeight: 900, padding: '3px 8px', borderRadius: '4px', backgroundColor: '#E8F5E9', color: '#2E7D32' }}>ENTREGADA</span></div>{a.notas_entrega && <div style={{ padding: '10px', backgroundColor: '#F5F5F7', borderRadius: '6px', marginBottom: '10px' }}><p style={{ fontSize: '12px', color: '#1D1D1F', margin: 0, lineHeight: '1.5' }}>{a.notas_entrega}</p></div>}{a.documento_entregable?.url && <a href={a.documento_entregable.url} target="_blank" rel="noreferrer" style={{ display: 'inline-block', padding: '10px 16px', backgroundColor: '#0071E3', color: '#FFFFFF', borderRadius: '6px', fontSize: '12px', fontWeight: 700, textDecoration: 'none', marginTop: '4px' }}>Descargar {a.documento_entregable.nombre_archivo}</a>}{a.respuesta_texto && a.nivel_servicio === 'Nivel 0' && <p style={{ fontSize: '14px', lineHeight: '1.6', margin: '8px 0 0 0', color: '#6E6E73' }}>{a.respuesta_texto}</p>}</div>)}</div>}
    </div>
  );
}
...
function PanelCDirector({ currentUser, userProfile, onCase, onConfig, onTeam, onLogout, onConsultas, onAssign, onBiblioteca, onPagos }: any) {
  ...
  return (
    ...
    <button onClick={onConsultas} style={{ ...styles.btnPrimary, marginBottom: '9px' }}>Ver todas las consultas</button>
      ...
      <label style={styles.label}>CASOS RECIENTES</label><div style={{ display: 'flex', gap: '10px', margin: '10px 0' }}><button onClick={() => { setVerArchivados(false); fetchData(); }} style={{ padding: '6px 14px', fontSize: '11px', fontWeight: 900, borderRadius: '4px', border: '1px solid #C0392B', backgroundColor: verArchivados ? 'transparent' : '#C0392B', color: verArchivados ? '#C0392B' : '#FFFFFF', cursor: 'pointer', flex: 1 }}>ACTIVOS</button><button onClick={() => { setVerArchivados(true); fetchData(); }} style={{ padding: '6px 14px', fontSize: '11px', fontWeight: 900, borderRadius: '4px', border: '1px solid #C0392B', backgroundColor: verArchivados ? '#C0392B' : 'transparent', color: verArchivados ? '#FFFFFF' : '#C0392B', cursor: 'pointer', flex: 1 }}>ARCHIVADOS</button></div>      {loading ? <p style={{ color: THEME.gray }}>Cargando casos...</p> : (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              {casos.length === 0 ? (
            <div style={{ ...styles.cardInfo, border: THEME.border, textAlign: 'center' }}>
              ...
              <div key={c.id} style={{ ...styles.itemCase, border: THEME.border, backgroundColor: colors.bg }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    {c.arquitecto_nombre && <p style={{ fontSize: '11px', color: THEME.gray, marginTop: '2px' }}>Arq. {c.arquitecto_nombre}</p>}
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'stretch', gap: '8px' }}>
                    {c.fecha_creacion && c.estado !== 'RESPONDIDA' && (() => { const horas = Math.floor((new Date().getTime() - (c.fecha_creacion.toDate ? c.fecha_creacion.toDate() : new Date(c.fecha_creacion)).getTime()) / (1000 * 60 * 60)); return <span style={{ fontSize: '10px', fontWeight: 900, padding: '3px 8px', borderRadius: '4px', border: horas < 24 ? '1px solid #2E7D32' : horas < 48 ? '1px solid #F57F17' : '1px solid #B21F24', backgroundColor: horas < 24 ? '#E8F5E9' : horas < 48 ? '#FFFDE7' : '#FFEBEE', color: horas < 24 ? '#2E7D32' : horas < 48 ? '#F57F17' : '#B21F24', textAlign: 'center' }}>{horas < 24 ? 'En plazo' : horas < 48 ? horas + 'hs' : 'VENCIDO'}</span>; })()}<span style={{ fontSize: '10px', fontWeight: 900, color: colors.text, border: `1px solid ${colors.text}`, padding: '4px 8px', borderRadius: '4px', textAlign: 'center' }}>{c.estado}</span>{!verArchivados && <button onClick={() => archivarCaso(c.id, true)} style={{ fontSize: '10px', fontWeight: 900, backgroundColor: 'transparent', color: '#6E6E73', border: '1px solid #6E6E73', padding: '4px 8px', borderRadius: '4px', cursor: 'pointer', width: '100%' }}>ARCHIVAR</button>}{verArchivados && <button onClick={() => archivarCaso(c.id, false)} style={{ fontSize: '10px', fontWeight: 900, backgroundColor: 'transparent', color: '#6E6E73', border: '1px solid #6E6E73', padding: '4px 8px', borderRadius: '4px', cursor: 'pointer', width: '100%' }}>DESARCHIVAR</button>}{verArchivados && <button onClick={() => borrarCaso(c.id)} style={{ fontSize: '10px', fontWeight: 900, backgroundColor: 'transparent', color: '#B21F24', border: '1px solid #B21F24', padding: '4px 8px', borderRadius: '4px', cursor: 'pointer', width: '100%' }}>BORRAR</button>}
                                      </div>
                </div>
              </div>
            );
...
function PanelADashboard({ currentUser, userProfile, onCase, onLogout, onBiblioteca, onMisCasos, onMiCuenta, onMiPerfil }: any) {
  ...
  return (
    <div style={styles.container}>
      ...
      {/* Casos recientes */}
      <label style={styles.label}>CASOS RECIENTES</label><div style={{ display: 'flex', gap: '10px', margin: '10px 0' }}><button onClick={() => { setVerArchivados(false); fetchCasos(); }} style={{ padding: '6px 14px', fontSize: '11px', fontWeight: 900, borderRadius: '4px', border: '1px solid #B21F24', backgroundColor: verArchivados ? 'transparent' : '#B21F24', color: verArchivados ? '#B21F24' : '#FFFFFF', cursor: 'pointer', flex: 1 }}>ACTIVOS</button><button onClick={() => { setVerArchivados(true); fetchCasos(); }} style={{ padding: '6px 14px', fontSize: '11px', fontWeight: 900, borderRadius: '4px', border: '1px solid #B21F24', backgroundColor: verArchivados ? '#B21F24' : 'transparent', color: verArchivados ? '#FFFFFF' : '#B21F24', cursor: 'pointer', flex: 1 }}>ARCHIVADOS</button></div>
...
function PanelBFicha({ caseId, onBack, onAdvanced, isDirectorView }: any) {
  ...
  const [actuacionEntregando, setActuacionEntregando] = useState<string|null>(null);
  useEffect(() => { if (!seleccionado) return; const q = query(collection(db,'Usuarios'), where('rol','==','arquitecto'), where('estudio_id','==',ESTUDIO_ID)); getDocs(q).then(snap => setArquitectos(snap.docs.map(d => ({id:d.id,...d.data()})))); }, [seleccionado]); const asignarArquitecto = async (arquitectoId: string, arquitectoNombre: string) => { if (!seleccionado) return; setAsignando(true); try { await updateDoc(doc(db,'Estudios',ESTUDIO_ID,'Casos',seleccionado),{arquitecto_asignado:arquitectoId,arquitecto_nombre:arquitectoNombre,estado:'EN AN\u00c1LISIS'}); setCaso((prev: any) => ({...prev,arquitecto_asignado:arquitectoId,arquitecto_nombre:arquitectoNombre,estado:'EN AN\u00c1LISIS'})); setSeleccionado(null); try { const arqSnap = await getDoc(doc(db,'Usuarios',arquitectoId)); console.log('arqSnap exists:', arqSnap.exists(), 'data:', arqSnap.data()); if (arqSnap.exists()) { const emailjs = await import('@emailjs/browser'); const r = await emailjs.send('delamatriz','template_no31o7y',{asunto:'Nuevo caso asignado - De La Matriz',mensaje:'Hola '+arquitectoNombre+', se te asigno un nuevo caso.\n\nInmueble: '+(caso?.direccion_inmueble||'')+'\nDescripcion: '+(caso?.descripcion||''),destinatario:arqSnap.data().email},'d1aTzq_ytY2X8Mrdn'); console.log('emailjs result:', r); } } catch(emailErr) { console.error('Email arquitecto:',emailErr); } } catch(e) { console.error('Error asignando:',e); } setAsignando(false); };
  useEffect(() => {
    if (!caseId) return;
    getDoc(doc(db, 'Estudios', ESTUDIO_ID, 'Casos', caseId)).then(d => {
      if (d.exists()) {
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px', marginTop: '15px' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
          <div style={{ ...styles.cardInfo, border: THEME.border }}>
            ...
            {isDirectorView && actuaciones.length > 0 && <div style={{...styles.cardInfo,border:'2px solid #1D1D1F',marginTop:'15px'}}><label style={{...styles.label,color:'#B21F24'}}>HISTORIAL DE ACTUACIONES</label>{actuaciones.map(a => <div key={a.id} style={{borderTop:'1px solid #E5E5E7',paddingTop:'10px',marginTop:'10px'}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'6px'}}><p style={{fontSize:'11px',fontWeight:900,color:'#6E6E73',margin:0}}>{a.nivel_servicio} — {a.nombre_servicio||''}</p><span style={{fontSize:'11px',fontWeight:900,padding:'3px 8px',borderRadius:'4px',backgroundColor:a.estado==='entregada'?'#E8F5E9':a.estado==='en_proceso'?'#FFF3E0':a.estado==='pago_confirmado'?'#E3F2FD':'#F5F5F7',color:a.estado==='entregada'?'#2E7D32':a.estado==='en_proceso'?'#FF9500':a.estado==='pago_confirmado'?'#0071E3':'#6E6E73'}}>{a.estado==='entregada'?'ENTREGADA':a.estado==='en_proceso'?'EN PROCESO':a.estado==='pago_confirmado'?'PAGO CONFIRMADO':a.estado==='completada'?'COMPLETADA':'PENDIENTE'}</span></div>{a.notas_entrega && <div style={{padding:'8px',backgroundColor:'#F5F5F7',borderRadius:'6px',marginBottom:'8px'}}><p style={{fontSize:'12px',color:'#1D1D1F',margin:0,lineHeight:'1.5'}}>{a.notas_entrega}</p></div>}{a.documento_entregable?.url && <a href={a.documento_entregable.url} target="_blank" rel="noreferrer" style={{display:'inline-block',padding:'8px 14px',backgroundColor:'#F5F5F7',color:'#0071E3',borderRadius:'6px',fontSize:'12px',fontWeight:700,textDecoration:'none',marginBottom:'8px',border:'1px solid #E5E5E7'}}>Ver documento: {a.documento_entregable.nombre_archivo}</a>}<div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}><span style={{fontSize:'11px',color:a.pago_usuario==='pagado'?'#2E7D32':'#E65100',fontWeight:700}}>{a.nivel_servicio==='Nivel 0'?'GRATUITO':a.pago_usuario==='pagado'?'PAGADO':('PAGO PENDIENTE — '+(a.metodo_pago||''))}</span>{a.fecha_entrega && <span style={{fontSize:'11px',color:'#6E6E73'}}>{new Date(a.fecha_entrega?.seconds*1000).toLocaleDateString('es-UY')}</span>}</div></div>)}</div>}
            {isDirectorView && caso && <button onClick={()=>generarPDFCaso(caso,actuaciones)} style={{marginTop:'15px',width:'100%',padding:'12px',backgroundColor:'#C0392B',color:'#fff',border:'none',borderRadius:'6px',fontSize:'13px',fontWeight:700,cursor:'pointer'}}>Descargar expediente PDF</button>}
            {!isDirectorView && actuaciones.length > 0 && <div style={{...styles.cardInfo,border:'2px solid #1D1D1F',marginTop:'15px'}}><label style={{...styles.label,color:'#B21F24'}}>HISTORIAL DE ACTUACIONES</label>{actuaciones.map(a => <div key={a.id} style={{borderTop:'1px solid #E5E5E7',paddingTop:'10px',marginTop:'10px'}}><p style={{fontSize:'11px',fontWeight:900,color:'#6E6E73',margin:'0 0 4px 0'}}>{a.nivel_servicio}{' — '}{a.nombre_servicio||''}</p><p style={{fontSize:'13px',margin:'0 0 6px 0'}}>{a.respuesta_texto||'Sin respuesta aun'}</p>{a.estado==='pago_confirmado' && <button onClick={async()=>{await tomarActuacion(caseId,a.id);setActuaciones(prev=>prev.map(x=>x.id===a.id?{...x,estado:'en_proceso'}:x));}} style={{marginTop:'10px',width:'100%',padding:'10px',backgroundColor:'#0071E3',color:'#fff',border:'none',borderRadius:'6px',fontSize:'12px',fontWeight:700,cursor:'pointer'}}>TOMAR ACTUACION</button>}{a.estado==='en_proceso' && <div style={{marginTop:'10px'}}><p style={{fontSize:'11px',fontWeight:900,color:'#FF9500',margin:'0 0 8px 0'}}>EN PROCESO</p><textarea placeholder="Mensaje al usuario (opcional)..." value={actuacionEntregando===a.id?notasEntrega:''} onChange={e=>{setActuacionEntregando(a.id);setNotasEntrega(e.target.value);}} style={{width:'100%',padding:'8px',border:'1px solid #E5E5E7',borderRadius:'6px',fontSize:'12px',minHeight:'70px',boxSizing:'border-box',marginBottom:'8px',resize:'none'}} />
            <button onClick={async()=>{const msg=(actuacionEntregando===a.id?notasEntrega:'');if(!msg.trim())return;try{const ejs=await import('@emailjs/browser');await ejs.send('delamatriz','template_no31o7y',{asunto:'Mensaje de tu arquitecto - De La Matriz',mensaje:'Hola '+(caso?.usuario_nombre||'')+'!\n\n'+msg+'\n\nIngresa a tu panel para mas informacion.',destinatario:caso?.usuario_email||''},'d1aTzq_ytY2X8Mrdn');alert('Mensaje enviado.');}catch(e){alert('Error al enviar.');}}} style={{width:'100%',padding:'8px',backgroundColor:'#E3F2FD',color:'#0071E3',border:'1px solid #0071E3',borderRadius:'6px',fontSize:'12px',fontWeight:700,cursor:'pointer',marginBottom:'8px'}}>Enviar mensaje al usuario</button>
            <UploadDocumento estudioId={ESTUDIO_ID} caseId={caseId} actuacionId={a.id} onUploadComplete={async(url,nombre,tamano)=>{const doc_entregable=construirDocumentoEntregable(url,nombre,tamano,a.arquitecto_id||'');await entregarActuacion(caseId,a.id,doc_entregable,notasEntrega);setActuaciones(prev=>prev.map(x=>x.id===a.id?{...x,estado:'entregada',documento_entregable:{url,nombre_archivo:nombre}}:x));try{const emailjs=await import('@emailjs/browser');await emailjs.send('delamatriz','template_no31o7y',{asunto:'Tu actuacion fue entregada - De La Matriz',mensaje:'Hola '+(caso?.usuario_nombre||'')+'!\n\nTu actuacion '+a.nombre_servicio+' fue completada y esta disponible en tu panel.\n\n'+(notasEntrega?'Mensaje del arquitecto: '+notasEntrega+'\n\n':'')+'Ingresa a tu panel para ver y descargar el documento.',destinatario:caso?.usuario_email||''},'d1aTzq_ytY2X8Mrdn');}catch(emailErr){console.error('Email entrega:',emailErr);}}} onError={e=>alert(e)} /><button onClick={async()=>{setEntregando(true);await entregarActuacion(caseId,a.id,null,notasEntrega);setActuaciones(prev=>prev.map(x=>x.id===a.id?{...x,estado:'entregada'}:x));try{const emailjs=await import('@emailjs/browser');await emailjs.send('delamatriz','template_no31o7y',{asunto:'Tu actuacion fue entregada - De La Matriz',mensaje:'Hola '+(caso?.usuario_nombre||'')+'!\n\nTu actuacion '+a.nombre_servicio+' fue completada.\n\n'+(notasEntrega?'Mensaje del arquitecto: '+notasEntrega+'\n\n':'')+'Ingresa a tu panel para ver el estado de tu consulta.',destinatario:caso?.usuario_email||''},'d1aTzq_ytY2X8Mrdn');}catch(emailErr){console.error('Email entrega:',emailErr);}setEntregando(false);}} disabled={entregando} style={{marginTop:'6px',width:'100%',padding:'10px',backgroundColor:'#6E6E73',color:'#fff',border:'none',borderRadius:'6px',fontSize:'12px',fontWeight:700,cursor:'pointer',opacity:entregando?0.7:1}}>MARCAR COMO ENTREGADA SIN DOCUMENTO</button></div>}{a.estado==='entregada' && <div style={{marginTop:'10px',padding:'10px',backgroundColor:'#E8F5E9',borderRadius:'6px',border:'1px solid #2E7D32'}}><p style={{color:'#2E7D32',fontWeight:900,fontSize:'12px',margin:'0 0 4px 0'}}>ENTREGADA</p>{a.documento_entregable?.url && <a href={a.documento_entregable.url} target="_blank" rel="noreferrer" style={{fontSize:'12px',color:'#0071E3',fontWeight:700}}>{a.documento_entregable.nombre_archivo}</a>}</div>}<div style={{display:'flex',justifyContent:'space-between',marginTop:'8px'}}><span style={{fontSize:'11px',fontWeight:700,color:a.pago_arquitecto==='pagado'?'#2E7D32':'#E65100'}}>{a.pago_arquitecto==='pagado'?'HONORARIO PAGADO':'HONORARIO PENDIENTE'}</span><span style={{fontSize:'11px',color:'#6E6E73'}}>{a.honorario_arquitecto?'$'+a.honorario_arquitecto:''}</span></div></div>)}</div>}            {caso?.estado !== 'RESPONDIDA' && caso?.estado !== 'FINALIZADA SIN PAGO' && !isDirectorView && !actuaciones.some(a => a.estado === 'en_proceso' || a.estado === 'entregada') && (<div style={{...styles.cardInfo,border:'2px solid #1D1D1F',marginTop:'15px'}}><label style={{fontSize:'11px',fontWeight:900,letterSpacing:'0.1em',color:'#1D1D1F',display:'block',marginBottom:'8px'}}>CONTACTO INICIAL CON EL USUARIO</label><p style={{fontSize:'11px',color:'#6E6E73',margin:'0 0 10px 0'}}>Enviá un mensaje al usuario confirmando que ya estas trabajando en su caso.</p>{contactoEnviado || caso?.contacto_inicial_enviado ? (<div style={{padding:'10px',backgroundColor:'#E8F5E9',borderRadius:'6px',border:'1px solid #2E7D32'}}><p style={{color:'#2E7D32',fontWeight:700,fontSize:'13px',margin:0}}>✅ Mensaje de contacto enviado al usuario</p></div>) : (<><textarea value={mensajeContacto || ('Hola ' + (caso?.usuario_nombre||'') + ', soy ' + (caso?.arquitecto_nombre||'el arquitecto asignado') + ' y ya estoy analizando tu consulta sobre ' + (caso?.direccion_inmueble||'tu inmueble') + '. En breve me pondre en contacto para confirmar los plazos de entrega. Ante cualquier duda podes responder este mensaje.')} onChange={e => setMensajeContacto(e.target.value)} style={{width:'100%',padding:'10px',border:'1px solid #E5E5E7',borderRadius:'6px',fontSize:'12px',minHeight:'100px',boxSizing:'border-box',marginBottom:'10px',fontFamily:'inherit',resize:'none'}} /><button onClick={handleContactoInicial} disabled={enviandoContacto} style={{width:'100%',padding:'12px',backgroundColor:'#1D1D1F',color:'#FFFFFF',border:'2px solid #1D1D1F',borderRadius:'6px',fontSize:'12px',fontWeight:700,cursor:'pointer',opacity:enviandoContacto?0.7:1}}>{enviandoContacto ? 'Enviando...' : 'Enviar mensaje al usuario'}</button></>)}</div>)}       </div>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                ...
                <button
                  onClick={async () => {
                    ...
                    try {
                      ...
                      const fotoParts = await Promise.all((caso.fotos_urls||[]).slice(0,3).map(async (url:string) => { const r = await fetch(url); const blob = await r.blob(); const b64 = await new Promise<string>(res => { const fr = new FileReader(); fr.onload = () => res((fr.result as string).split(',')[1]); fr.readAsDataURL(blob); }); return { inlineData: { mimeType: blob.type || 'image/jpeg', data: b64 } }; }));
                      const res = await fetch('https://generativelanguage.googleapis.com/v1beta/models/gemini-flash-latest:generateContent', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json', 'X-goog-api-key': import.meta.env.VITE_GEMINI_API_KEY },
                        body: JSON.stringify({ contents: [{ parts: [...fotoParts, { text: prompt }] }] })
                      });
...
function PanelB1Tablero({ caseId, onBack, onUserView }: any) {
  return (
    <div style={styles.container}>
      <div style={{ ...styles.engineeringHeader, justifyContent: 'space-between', alignItems: 'center' }}><button onClick={onBack} style={styles.btnBack}>← Volver</button><span>Tablero Técnico</span></div>
...
function PanelGConsultas({ onCase, onBack }: any) {
  ...
  return (
    ...
    <div style={styles.container}>
      <div style={{ ...styles.engineeringHeader, justifyContent: 'space-between', alignItems: 'center' }}><button onClick={onBack} style={styles.btnBack}>← Volver</button><span>Historial Global</span></div>
